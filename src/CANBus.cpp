...
CANBus::CANBus(ros::NodeHandle &NodeHandle):nh_(NodeHandle)
{
    ...
    // plane_sub_ = nh_.subscribe("/detection/image_detector/person", 1, &CANBus::planeCallback, this); //视觉检测目标话题订阅
...
// void CANBus::ctrlCallback(const autoware_msgs::ControlCommandStamped& msg)
// {
//     ctrl_call_++;
//     can_send_.clear();
...
//     double diff = msg.cmd.linear_velocity - vel_;
//     double ts = 0.01;
//     double calib_val = longctrl_.pidcontroller(diff,ts,vel_); // pid controller
...
//     double str_cmd = msg.cmd.steering_angle;//right为正
...
// void CANBus::planeCallback(const std_msgs::Int8& target_msg)
// {
 
//     can_sendX.clear();
//     can_bus::CanFrame openplane116;

//     openplane116.id = 0x116;
//     openplane116.len = 2;
//     uint8_t* openplane_data = openplane116.data;

//     uint8_t plane_cmd = 0;
//     if(target_msg.data ==1)
//     {   
//         count++;
//     }
//     else 
...
//     else{
//         plane_cmd = 0;
//          ROS_INFO("detect no person");

//     }
    

//     //对发送的指令进行赋值
//     Byte plane_ctrl (openplane_data+0);
//     plane_ctrl.set_value(static_cast<uint8_t> (plane_cmd),0,8);
//     can_sendX.push_back(openplane116);
//     can_client_ ->Send(can_sendX, &send_frame1_);
//     can_sendX.clear();
...
void CANBus::timerCallback(const ros::TimerEvent& event)
{
    ...
    can_info_.header.frame_id= "/can_info";
