...
class Teleop {
 public:
  Teleop(ros::NodeHandle& nh):nh_(nh){
      ctr_pub_ = nh_.advertise<autoware_msgs::ControlCommandStamped>("/ctrl_cmd",1) ;
    ...
    while (ros::ok()) {
      ...
      switch (c) {
        ...
        case KEYCODE_UP2:
          cmd_.cmd.linear_velocity += throttle_inc_delta;
          cmd_.cmd.linear_velocity = cmd_.cmd.linear_velocity>1?1:cmd_.cmd.linear_velocity;
          cout<<"target linear_velocty command = "<<cmd_.cmd.linear_velocity<<endl;
          break;
        
        case KEYCODE_DN1:  // decelerate
        case KEYCODE_DN2:
           cmd_.cmd.linear_velocity -= brake_inc_delta;
          cmd_.cmd.linear_velocity = cmd_.cmd.linear_velocity>0?cmd_.cmd.linear_velocity:0;
            cout<<"target linear_velocity command ="<<cmd_.cmd.linear_velocity<<endl;
          break;

        case KEYCODE_LF1:  // left
        case KEYCODE_LF2:
          cmd_.cmd.steering_angle += steer_inc_delta;
          cmd_.cmd.steering_angle = cmd_.cmd.steering_angle>-0.5?cmd_.cmd.steering_angle:-0.5;
          cout << "Steering Target command = " << cmd_.cmd.steering_angle<<endl;
          break;
        
        case KEYCODE_RT1:  // right
        case KEYCODE_RT2:
          cmd_.cmd.steering_angle -= steer_inc_delta;
          cmd_.cmd.steering_angle = cmd_.cmd.steering_angle>0.5?0.5:cmd_.cmd.steering_angle;
          cout << "Steering Target command = " << cmd_.cmd.steering_angle<<endl;
          break;
        ...
        case KEYCODE_ESTOP1:
        case KEYCODE_ESTOP2:
          cmd_.cmd.linear_velocity = -0.5;
          cout << "Estop Brake : " << cmd_.cmd.linear_acceleration<<endl;
          break;

        case KEYCODE_SETT1:  // set throttle
        case KEYCODE_SETT2:
          
          cmd_.cmd.linear_velocity +=  1;
         cout<<"Target llinearvelocity is: "<<cmd_.cmd.linear_velocity<<endl;
          break;
...
private:
    ...
    ros::Timer timer_;
    autoware_msgs::ControlCommandStamped cmd_;
    ...
    void resetkeyboard()
    {
        cmd_.cmd.linear_acceleration = 0.0;
        cmd_.cmd.steering_angle = 0.0;
        cmd_.cmd.linear_velocity = 0.0;
    }
};
